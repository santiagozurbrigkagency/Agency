...
Cerrada por Santi el 6 de septiembre. Es la que va a la bio, al pitch y a la llamada de venta.

NOMBRE DE LA OFERTA
  Génesis — Escalar Hacia Adentro. Consultoría 1a1.
  La frase vende por categoría ("modelo de suscripción"); el encabezado y el cierre llevan el
...
NO VOLVER A
  "marca de consumibles" en lugar de "ecomm de consumibles". Se probó y se descartó: "marca" abre
  el filtro a quien vende en retail, mayorista o Mercado Libre sin tienda propia, y los entregables
  3 y 4 (checkout recurrente, conversión de la base) asumen tienda propia y base de compradores.
  Además "ecom" es la categoría por la que el avatar busca; "marca" no se busca.
...
DOLOR 1 — "sin depender del CPA"
  ...
  Nota para el discurso: el método no elimina el CPA, permite pagar más. Cuando se explica en la call o en contenido, la formulación precisa es "sin pelear el CPA más bajo" — porque el argumento es la subasta: no competís por el CPM más bajo, competís por quién puede pagar más por el mismo cliente.

DOLOR 2 — "ni dejar de vender"
  Sale del obstáculo O2 del método: no frenar ni romper la operación que hoy factura mientras cambia el modelo. Es el miedo específico del Nivel 3.
  Desde que está en la oferta, dejó de ser un argumento y pasó a ser una obligación de entrega: la suscripción se instala como variante paralela y la compra única no se toca. Está resuelto en la pieza 3 del DFY.

  DIVERGENCIA ABIERTA CON EL AVATAR [DECISIÓN PENDIENTE]
    Avatar-Genesis.md dice que los dos "sin" deben ser el Dolor 2 (más pauta) y el Dolor 6
    (cambiar de producto → "sin depender del producto que pegue"). Esta oferta usa el Dolor 2 y el O2.
    El segundo "sin" no coincide entre los dos documentos.
      "ni dejar de vender"                  → desarma el miedo a romper la operación (O2).
      "sin depender del producto que pegue" → ataca la objeción a comprar otra mentoría (Dolor 6).
    Los dos son legítimos. Se resuelve con el Typeform o preguntándole a Manu qué escucha más en
    las llamadas. Hasta entonces queda "ni dejar de vender", que es la versión cerrada.

MÉTODO ÚNICO — "convirtiéndolo a modelo de suscripción"
  En la oferta va la categoría, no el nombre del método. Es correcto para que se entienda y se busque: el avatar está consciente de la solución pero no del mecanismo.
  El nombre del método —Escalar Hacia Adentro— va donde se recuerda: encabezado y cierre de la página de oferta, bio, cierre de reel, nombre del programa, webinar, SOPs.
  Regla: se busca por categoría, se recuerda por método.

============================================================
3. LA OFERTA COMO SE PRESENTA
============================================================

Página completa: lo que ve el prospecto en la call, en el PDF y —recortada— en el webinar.
Los 12 renglones de "qué incluye" mapean 1:1 con las siete piezas del método. El mapeo vive en
Metodo-Unico-Genesis.md, sección "Las piezas en la entrega DFY". Acá va el copy, no el mapeo.
...
------------------------------------------------------------
  ...
  QUÉ INCLUYE
    ...
    Tu variante de suscripción definida, precificada y validada contra tu ciclo de
      reposición real: producto, formato y qué gana el que se suscribe (DFY)
    ...
    Tu conversión de la base actual: la secuencia por mail y WhatsApp para pasar a
      suscripción a los clientes que ya te compraron (DFY)
...
NOTAS DE COPY
  ...
  Prueba de Meta y Ventaja Invisible son consecuencia del conjunto, no módulos. Se cobran en copy,
  no inventando piezas.

============================================================
4. LA BIO DE INSTAGRAM
============================================================

Estructura de 5 líneas del SOP: resultado → sin dolores → autoridad → CTA → método.

  Ecomm de consumibles a +$100M/mes
...
Notas:
  Línea 1 vende el destino. Mayúsculas en SUSCRIPCIÓN y en el método — regla del SOP de mayúsculas estratégicas.
  Línea 3 es la autoridad. Son los dos números que ningún competidor hispano puede mostrar: ninguno publica una métrica de retención.
  "ADENTRO" como palabra clave: única en el mercado (los competidores usan ECOM, MARCA, ESA, MAP, IA) y repite el método cada vez que alguien la escribe. [CONFIRMAR MANU]
  Link: el video largo de suscripciones cuando esté editado; después, lead magnet o VSL. [DEFINIR]
  TikTok recortado: "Ecomm de consumibles a +$100M/mes con suscripción. DM ADENTRO."

BLOQUE DE AUTORIDAD, al final de cada caption (costo cero, cero implementaciones en el mercado hispano)
  "Manu. Escalo hacia adentro: 2.200 suscriptores activos, 1,28% de churn. Enseño el modelo en Génesis."

============================================================
5. VERSIONES DEL PITCH POR USO
============================================================
  ...
  Título y descripción de YouTube — categoría adelante ("ecom de suscripción") por búsqueda, método en el cuerpo.
  Cierre de reel y de video largo — "Esto es escalar hacia adentro."
...
SI SE COBRA EN CUOTAS
  ...
  "una vez instalado el DFY". Con 3 cuotas, alguien paga 1.700, se lleva la tienda instalada, la
  base convertida y las campañas recalculadas, y no aparece más.
  Regla: el entregable 3 (tienda con suscripción instalada) no se instala hasta que esté paga la
  cuota 2. Es el más caro de producir y el más fácil de llevarse. Se dice de frente en la call, no
  en letra chica.
...
Son dos validaciones distintas:
  ...
  VALIDACIÓN 2 — EL MÉTODO
    Prueba: que el método produce el resultado prometido.
    Cuándo: el primer cobro recurrente de cada cliente, a los 30 días de su arranque. No hace falta esperar los 3 meses.
    Sí: 3 de 5 con primer cobro recurrente en el mes 1.
    No: se atrasan todos en el mismo punto. Ese punto es lo que hay que resolver antes de escalar — probablemente el cobro recurrente o el ciclo de reposición.
    Decide: si la promesa de los 30 días se sostiene o se cambia por otra velocidad.
...
Regla operativa del SOP: cada 2 o 3 meses se reconstruye la oferta de cero y se compara con esta. O se confirma, o aparece una mejora.
...
============================================================
  ...
  2. Ciclo de reposición típico de los productos del avatar. Define si sale el renglón "primer cobro
     al cumplirse tu ciclo de reposición", que es lo único que hoy le falta al bloque de precio.
  3. Cuánto tarda la instalación completa del DFY. Define el [N] de sesiones 1a1. Sin el número, la
     página no sale.
